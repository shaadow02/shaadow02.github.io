...
game.Players.PlayerRemoving:Connect(function(player) if player.Name == getgenv().Owner then local Players = game:GetService("Players") local TeleportService = game:GetService("TeleportService") local player = Players.LocalPlayer local userId = player.UserId if userId == alt1 or userId == alt2 then local function checkFriendInGame() local friends = player:GetFriendsOnline(200) local friend = friends[1] if not friend then return end local placeId = friend.PlaceId if tonumber(placeId) == 2788229376 then if friend.GameId then TeleportService:TeleportToPlaceInstance(2788229376, friend.GameId, player) else TeleportService:Teleport(2788229376, player) end else end end checkFriendInGame() while true do wait(5) checkFriendInGame() end end end end) local ownerPlayer = game.Players:FindFirstChild(getgenv().Owner) if not ownerPlayer then local Players = game:GetService("Players") local TeleportService = game:GetService("TeleportService") local player = Players.LocalPlayer local userId = player.UserId if userId == alt1 or userId == alt2 then local function checkFriendInGame() local friends = player:GetFriendsOnline(200) local friend = friends[1] if not friend then return end local placeId = friend.PlaceId if tonumber(placeId) == 2788229376 then if friend.GameId then TeleportService:TeleportToPlaceInstance(2788229376, friend.GameId, player) else TeleportService:Teleport(2788229376, player) end else end end checkFriendInGame() while true do wait(5) checkFriendInGame() end end end
  
getgenv().Configuration = {
['Msg'] = 'shdw sigma.', --When You Say [/e msg] It Makes All The Alts Say That Message
['CrewID'] = 6674901, --Must join group
...
['Position'] = 'Right', --[Back | Left | Right | Mid | UpMid | UpLeft | UpLeft | Walk ]
...
elseif userId == alt2 and placeId == 2788229376 then
getgenv()._=
"Join discord.gg/msgabv2t9Q | discord.gg/stando to get latest update ok bai OwO_ | If you pay for this script you get scammed, this script is completely free ok"
